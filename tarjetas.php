...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['accion']) && $_POST['accion'] === 'agregar') {
    ...
    if ($texto && $sector && $carrera) {
        if (mysqli_query($conexion, "INSERT INTO tarjetas (texto, sector, carrera, imagen_url) VALUES ('$texto','$sector','$carrera','$imagen_url')")) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['accion']) && $_POST['accion'] === 'editar') {
    ...
    $carrera = mysqli_real_escape_string($conexion, trim($_POST['carrera']));
    $imagen_url = mysqli_real_escape_string($conexion, trim($_POST['imagen_url']));
    if (mysqli_query($conexion, "UPDATE tarjetas SET texto='$texto', sector='$sector', carrera='$carrera', imagen_url='$imagen_url' WHERE id=$id")) {
...
$tarjetas = mysqli_query($conexion, "SELECT * FROM tarjetas $where ORDER BY sector, carrera, id");
$total = mysqli_num_rows($tarjetas);
?>
...
<div class="contenedor-layout">
    <nav class="menu-lateral">
        ...
        <ul>
            ...
            <li><a href="tarjetas.php" class="activo">🃏 Tarjetas</a></li>
    ...
    <main class="contenido-derecha" style="align-items:flex-start; padding:30px;">
        <div style="width:100%;">
            ...
            <div class="preg-header">
                <div>
                    <h2 style="color:#1B2E5E; margin:0;">🃏 Gestión de Tarjetas Vocacionales</h2>
                    <p style="color:#888; font-size:13px; margin:4px 0 0;">Administra las tarjetas que verán los alumnos en el test de la app móvil.</p>
                </div>
                <button class="btn-agregar" onclick="abrirModal()">+ Nueva Tarjeta</button>
            </div>

            <!-- FILTROS -->
            <form method="GET" action="tarjetas.php">
                <div class="filtros-bar">
                    <input type="text" name="buscar" placeholder="🔍 Buscar tarjeta..." value="<?= htmlspecialchars($busqueda) ?>">
                    <select name="sector">
                    ...
                    <a href="tarjetas.php" style="font-size:12px; color:#888; text-decoration:none;">✕ Limpiar</a>
                    <span class="badge-total"><?= $total ?> tarjeta(s)</span>
                </div>
            </form>
            ...
            <div class="caja-blanca" style="padding:0; overflow:hidden;">
                ...
                <table class="tabla-preguntas">
                    ...
                    <tbody>
                        <?php while ($t = mysqli_fetch_assoc($tarjetas)): ?>
                        <tr>
                            <td style="color:#aaa; font-size:12px;"><?= $t['id'] ?></td>
                            <td><?= htmlspecialchars($t['texto']) ?></td>
                            <td><span class="tag-sector"><?= htmlspecialchars($t['sector']) ?></span></td>
                            <td><span class="tag-carrera"><?= htmlspecialchars($t['carrera']) ?></span></td>
                            <td style="white-space:nowrap;">
                                <button class="btn-editar" onclick="abrirEditar(<?= $t['id'] ?>, <?= htmlspecialchars(json_encode($t['texto'])) ?>, <?= htmlspecialchars(json_encode($t['sector'])) ?>, <?= htmlspecialchars(json_encode($t['carrera'])) ?>, <?= htmlspecialchars(json_encode($t['imagen_url'])) ?>)">✏️ Editar</button>
                                <a href="tarjetas.php?eliminar=<?= $t['id'] ?>" onclick="return confirm('¿Eliminar esta tarjeta?')">
                ...
                <?php else: ?>
                    <div class="sin-resultados"><div style="font-size:40px;">🃏</div><p>No se encontraron tarjetas.</p></div>
                <?php endif; ?>
            </div>
